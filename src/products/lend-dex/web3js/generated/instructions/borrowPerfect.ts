...
export const BORROW_PERFECT_INSTRUCTION_DISCRIMINATOR = new Uint8Array([210, 67, 245, 255, 134, 200, 153, 58]);

export interface BorrowPerfectInstructionAccounts {
    signer: Address;
    dex: Address;
    user: Address;
    position: Address;
...
export interface BorrowPerfectInstructionArgs {
    shares: number | bigint;
    minToken0: number | bigint;
    minToken1: number | bigint;
}

function getBorrowPerfectInstructionDataEncoder(): Encoder<BorrowPerfectInstructionArgs> {
...
function getBorrowPerfectInstructionDataDecoder(): Decoder<BorrowPerfectInstructionArgs> {
...
export interface ParsedBorrowPerfectInstruction {
    programId: Address;
    accounts: {
        signer: AccountMeta;
        dex: AccountMeta;
    ...
    data: BorrowPerfectInstructionArgs;
}

export function parseBorrowPerfectInstruction(instruction: TransactionInstruction): ParsedBorrowPerfectInstruction {
    if (instruction.keys.length < 26) {
        throw new Error('Expected 26 account metas for BorrowPerfect instruction');
    }
    if (!BORROW_PERFECT_INSTRUCTION_DISCRIMINATOR.every((byte, index) => instruction.data[0 + index] === byte)) {
        throw new Error('BorrowPerfect instruction discriminator mismatch');
    }
    ...
    return {
        ...
        data: getBorrowPerfectInstructionDataDecoder().decode(instructionData),
    };
}

export function createBorrowPerfectInstruction(
    accounts: BorrowPerfectInstructionAccounts,
    args: BorrowPerfectInstructionArgs,
    ...
    let data = Buffer.from(getBorrowPerfectInstructionDataEncoder().encode(args));
    data = Buffer.concat([
        data.subarray(0, 0),
        Buffer.alloc(Math.max(0, 0 - data.length)),
        Buffer.from(BORROW_PERFECT_INSTRUCTION_DISCRIMINATOR),
        data.subarray(0),
    ]);
