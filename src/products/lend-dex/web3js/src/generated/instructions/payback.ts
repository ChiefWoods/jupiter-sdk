...
export interface PaybackInstructionArgs {
    token0Amt: bigint;
    token1Amt: bigint;
    minShares: bigint;
}

const PaybackInstructionDataCodec = getStructCodec([
    ['token0Amt', getU64Codec()],
    ['token1Amt', getU64Codec()],
    ['minShares', getU64Codec()],
]);
...
export function createPaybackInstruction(
    ...
    const instructionData = Buffer.from(PaybackInstructionDataCodec.encode(args));
