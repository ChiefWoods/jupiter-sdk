...
export const WITHDRAW_PERFECT_INSTRUCTION_DISCRIMINATOR = new Uint8Array([41, 170, 122, 166, 20, 154, 180, 245]);

export interface WithdrawPerfectInstructionAccounts {
    signer: Address;
    dex: Address;
    user: Address;
    position: Address;
...
export interface WithdrawPerfectInstructionArgs {
    shares: number | bigint;
    minToken0: number | bigint;
    minToken1: number | bigint;
}

function getWithdrawPerfectInstructionDataEncoder(): Encoder<WithdrawPerfectInstructionArgs> {
...
function getWithdrawPerfectInstructionDataDecoder(): Decoder<WithdrawPerfectInstructionArgs> {
...
export interface ParsedWithdrawPerfectInstruction {
    programId: Address;
    accounts: {
        signer: AccountMeta;
        dex: AccountMeta;
    ...
    data: WithdrawPerfectInstructionArgs;
}

export function parseWithdrawPerfectInstruction(instruction: TransactionInstruction): ParsedWithdrawPerfectInstruction {
    if (instruction.keys.length < 26) {
        throw new Error('Expected 26 account metas for WithdrawPerfect instruction');
    }
    if (!WITHDRAW_PERFECT_INSTRUCTION_DISCRIMINATOR.every((byte, index) => instruction.data[0 + index] === byte)) {
        throw new Error('WithdrawPerfect instruction discriminator mismatch');
    }
    ...
    return {
        ...
        data: getWithdrawPerfectInstructionDataDecoder().decode(instructionData),
    };
}

export function createWithdrawPerfectInstruction(
    accounts: WithdrawPerfectInstructionAccounts,
    args: WithdrawPerfectInstructionArgs,
    ...
    let data = Buffer.from(getWithdrawPerfectInstructionDataEncoder().encode(args));
    data = Buffer.concat([
        data.subarray(0, 0),
        Buffer.alloc(Math.max(0, 0 - data.length)),
        Buffer.from(WITHDRAW_PERFECT_INSTRUCTION_DISCRIMINATOR),
        data.subarray(0),
    ]);
