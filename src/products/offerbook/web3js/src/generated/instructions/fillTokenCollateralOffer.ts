...
const FillTokenCollateralOfferInstructionDataCodec = getStructCodec([
    ['collateralFillAmount', getU64Codec()],
    ['maxPrincipal', getU64Codec()],
...
export async function createFillTokenCollateralOfferInstruction(
    ...
    if (!borrowerCollateralEscrow) {
        const [derived] = await findBorrowerCollateralEscrowPda(
            {
                borrowerUser: accounts.borrowerUser,
    ...
    const instructionData = Buffer.from(FillTokenCollateralOfferInstructionDataCodec.encode(args));
