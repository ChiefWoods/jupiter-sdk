...
export const SWAP_OUT_INSTRUCTION_DISCRIMINATOR = new Uint8Array([206, 36, 149, 14, 163, 132, 148, 1]);

export interface SwapOutInstructionAccounts {
    signer: Address;
    dex: Address;
...
export interface SwapOutInstructionArgs {
    swap0to1: boolean;
    amountOut: number | bigint;
    amountInMax: number | bigint;
}

function getSwapOutInstructionDataEncoder(): Encoder<SwapOutInstructionArgs> {
    return getStructEncoder([
        ['swap0to1', getBooleanEncoder()],
        ['amountOut', getU64Encoder()],
        ['amountInMax', getU64Encoder()],
    ]);
}

function getSwapOutInstructionDataDecoder(): Decoder<SwapOutInstructionArgs> {
    return getStructDecoder([
        ['swap0to1', getBooleanDecoder()],
        ['amountOut', getU64Decoder()],
        ['amountInMax', getU64Decoder()],
    ]);
}

export interface ParsedSwapOutInstruction {
    programId: Address;
    accounts: {
        signer: AccountMeta;
        dex: AccountMeta;
    ...
    data: SwapOutInstructionArgs;
}

export function parseSwapOutInstruction(instruction: TransactionInstruction): ParsedSwapOutInstruction {
    if (instruction.keys.length < 24) {
        throw new Error('Expected 24 account metas for SwapOut instruction');
    }
    if (!SWAP_OUT_INSTRUCTION_DISCRIMINATOR.every((byte, index) => instruction.data[0 + index] === byte)) {
        throw new Error('SwapOut instruction discriminator mismatch');
    }
    ...
    return {
        ...
        data: getSwapOutInstructionDataDecoder().decode(instructionData),
    };
}

export function createSwapOutInstruction(
    accounts: SwapOutInstructionAccounts,
    args: SwapOutInstructionArgs,
    ...
    let data = Buffer.from(getSwapOutInstructionDataEncoder().encode(args));
    data = Buffer.concat([
        data.subarray(0, 0),
        Buffer.alloc(Math.max(0, 0 - data.length)),
        Buffer.from(SWAP_OUT_INSTRUCTION_DISCRIMINATOR),
        data.subarray(0),
    ]);
