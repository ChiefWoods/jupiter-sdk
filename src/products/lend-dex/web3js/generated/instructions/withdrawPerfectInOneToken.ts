...
export const WITHDRAW_PERFECT_IN_ONE_TOKEN_INSTRUCTION_DISCRIMINATOR = new Uint8Array([
    200, 72, 151, 197, 24, 15, 243, 165,
]);

export interface WithdrawPerfectInOneTokenInstructionAccounts {
    signer: Address;
    dex: Address;
    user: Address;
    position: Address;
...
export interface WithdrawPerfectInOneTokenInstructionArgs {
    shares: number | bigint;
    minToken0: number | bigint;
    minToken1: number | bigint;
}

function getWithdrawPerfectInOneTokenInstructionDataEncoder(): Encoder<WithdrawPerfectInOneTokenInstructionArgs> {
...
function getWithdrawPerfectInOneTokenInstructionDataDecoder(): Decoder<WithdrawPerfectInOneTokenInstructionArgs> {
...
export interface ParsedWithdrawPerfectInOneTokenInstruction {
    programId: Address;
    accounts: {
        signer: AccountMeta;
        dex: AccountMeta;
    ...
    data: WithdrawPerfectInOneTokenInstructionArgs;
}

export function parseWithdrawPerfectInOneTokenInstruction(
    instruction: TransactionInstruction,
): ParsedWithdrawPerfectInOneTokenInstruction {
    if (instruction.keys.length < 26) {
        throw new Error('Expected 26 account metas for WithdrawPerfectInOneToken instruction');
    }
    if (
        !WITHDRAW_PERFECT_IN_ONE_TOKEN_INSTRUCTION_DISCRIMINATOR.every(
            (byte, index) => instruction.data[0 + index] === byte,
        )
    ) {
        throw new Error('WithdrawPerfectInOneToken instruction discriminator mismatch');
    }
    ...
    return {
        ...
        data: getWithdrawPerfectInOneTokenInstructionDataDecoder().decode(instructionData),
    };
}

export function createWithdrawPerfectInOneTokenInstruction(
    accounts: WithdrawPerfectInOneTokenInstructionAccounts,
    args: WithdrawPerfectInOneTokenInstructionArgs,
    ...
    let data = Buffer.from(getWithdrawPerfectInOneTokenInstructionDataEncoder().encode(args));
    data = Buffer.concat([
        data.subarray(0, 0),
        Buffer.alloc(Math.max(0, 0 - data.length)),
        Buffer.from(WITHDRAW_PERFECT_IN_ONE_TOKEN_INSTRUCTION_DISCRIMINATOR),
        data.subarray(0),
    ]);
