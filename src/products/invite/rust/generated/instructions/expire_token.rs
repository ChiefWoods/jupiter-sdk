...
pub const EXPIRE_TOKEN_DISCRIMINATOR: [u8; 8] = [90, 208, 189, 130, 72, 252, 204, 11];

/// Accounts.
#[derive(Debug)]
pub struct ExpireToken {
...
impl ExpireToken {
    ...
    pub fn instruction_with_remaining_accounts(
        ...
        let data = ExpireTokenInstructionData::new().try_to_vec().unwrap();
...
pub struct ExpireTokenInstructionData {
    discriminator: [u8; 8],
}

impl ExpireTokenInstructionData {
    pub fn new() -> Self {
        Self {
            discriminator: [90, 208, 189, 130, 72, 252, 204, 11],
        }
    }
...
impl Default for ExpireTokenInstructionData {
    fn default() -> Self {
        Self::new()
    }
}

/// Instruction builder for `ExpireToken`.
///
/// ### Accounts:
///
...
///   7. `[writable]` mint
///   8. `[writable, signer]` keeper
#[derive(Clone, Debug, Default)]
pub struct ExpireTokenBuilder {
...
/// `expire_token` CPI accounts.
pub struct ExpireTokenCpiAccounts<'a, 'b> {
...
/// `expire_token` CPI instruction.
pub struct ExpireTokenCpi<'a, 'b> {
...
impl<'a, 'b> ExpireTokenCpi<'a, 'b> {
    pub fn new(
        program: &'b solana_account_info::AccountInfo<'a>,
        accounts: ExpireTokenCpiAccounts<'a, 'b>,
    ) -> Self {
        Self {
            __program: program,
...
/// Instruction builder for `ExpireToken` via CPI.
///
/// ### Accounts:
///
///   0. `[writable]` invite_info
...
///   8. `[writable, signer]` keeper
#[derive(Clone, Debug)]
pub struct ExpireTokenCpiBuilder<'a, 'b> {
    instruction: Box<ExpireTokenCpiBuilderInstruction<'a, 'b>>,
}

impl<'a, 'b> ExpireTokenCpiBuilder<'a, 'b> {
    pub fn new(program: &'b solana_account_info::AccountInfo<'a>) -> Self {
        let instruction = Box::new(ExpireTokenCpiBuilderInstruction {
            __program: program,
            invite_info: None,
            sender: None,
    ...
    #[allow(clippy::vec_init_then_push)]
    pub fn invoke_signed(&self, signers_seeds: &[&[&[u8]]]) -> solana_program_error::ProgramResult {
        let instruction = ExpireTokenCpi {
...
#[derive(Clone, Debug)]
struct ExpireTokenCpiBuilderInstruction<'a, 'b> {
