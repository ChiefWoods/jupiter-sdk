...
export const OPERATE_PERFECT_DEX_INSTRUCTION_DISCRIMINATOR = new Uint8Array([88, 34, 180, 77, 152, 186, 166, 162]);
...
function getOperatePerfectDexInstructionDataDecoder(): Decoder<OperatePerfectDexInstructionArgs> {
    return getStructDecoder([
        ['colAmounts', getOptionDecoder(getOperatePerfectDexColAmountsDecoder())],
        ['debtAmounts', getOptionDecoder(getOperatePerfectDexDebtAmountsDecoder())],
...
export interface ParsedOperatePerfectDexInstruction {
    programId: Address;
    accounts: {
        signer: AccountMeta;
    ...
    data: OperatePerfectDexInstructionArgs;
}

export function parseOperatePerfectDexInstruction(
    instruction: TransactionInstruction,
): ParsedOperatePerfectDexInstruction {
    if (instruction.keys.length < 73) {
        throw new Error('Expected 73 account metas for OperatePerfectDex instruction');
    }
    if (!OPERATE_PERFECT_DEX_INSTRUCTION_DISCRIMINATOR.every((byte, index) => instruction.data[0 + index] === byte)) {
        throw new Error('OperatePerfectDex instruction discriminator mismatch');
    }
    ...
    return {
        ...
        data: getOperatePerfectDexInstructionDataDecoder().decode(instructionData),
    };
}
...
export function createOperatePerfectDexInstruction(
    ...
    let data = Buffer.from(getOperatePerfectDexInstructionDataEncoder().encode(args));
    data = Buffer.concat([
        data.subarray(0, 0),
        Buffer.alloc(Math.max(0, 0 - data.length)),
        Buffer.from(OPERATE_PERFECT_DEX_INSTRUCTION_DISCRIMINATOR),
        data.subarray(0),
    ]);
