...
import {
    ...
    type LiquidateDexDebtAmountsArgs,
} from '../types/liquidateDexDebtAmounts';
import { getTransferTypeDecoder, getTransferTypeEncoder, type TransferTypeArgs } from '../types/transferType';

export const LIQUIDATE_DEX_INSTRUCTION_DISCRIMINATOR = new Uint8Array([28, 129, 253, 125, 243, 52, 11, 162]);

export interface LiquidateDexInstructionAccounts {
    signer: Address;
...
export interface LiquidateDexInstructionArgs {
    debtAmt: number | bigint;
    debtAmounts: OptionOrNullable<LiquidateDexDebtAmountsArgs>;
...
function getLiquidateDexInstructionDataEncoder(): Encoder<LiquidateDexInstructionArgs> {
    return getStructEncoder([
        ['debtAmt', getU64Encoder()],
        ['debtAmounts', getOptionEncoder(getLiquidateDexDebtAmountsEncoder())],
...
function getLiquidateDexInstructionDataDecoder(): Decoder<LiquidateDexInstructionArgs> {
    return getStructDecoder([
        ['debtAmt', getU64Decoder()],
        ['debtAmounts', getOptionDecoder(getLiquidateDexDebtAmountsDecoder())],
...
export interface ParsedLiquidateDexInstruction {
    programId: Address;
    accounts: {
        signer: AccountMeta;
    ...
    data: LiquidateDexInstructionArgs;
}

export function parseLiquidateDexInstruction(instruction: TransactionInstruction): ParsedLiquidateDexInstruction {
    if (instruction.keys.length < 67) {
        throw new Error('Expected 67 account metas for LiquidateDex instruction');
    }
    if (!LIQUIDATE_DEX_INSTRUCTION_DISCRIMINATOR.every((byte, index) => instruction.data[0 + index] === byte)) {
        throw new Error('LiquidateDex instruction discriminator mismatch');
    }
    ...
    return {
        ...
        data: getLiquidateDexInstructionDataDecoder().decode(instructionData),
    };
}

export function createLiquidateDexInstruction(
    accounts: LiquidateDexInstructionAccounts,
    args: LiquidateDexInstructionArgs,
    ...
    let data = Buffer.from(getLiquidateDexInstructionDataEncoder().encode(args));
    data = Buffer.concat([
        data.subarray(0, 0),
        Buffer.alloc(Math.max(0, 0 - data.length)),
        Buffer.from(LIQUIDATE_DEX_INSTRUCTION_DISCRIMINATOR),
        data.subarray(0),
    ]);
