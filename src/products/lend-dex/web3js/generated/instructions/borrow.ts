...
export const BORROW_INSTRUCTION_DISCRIMINATOR = new Uint8Array([228, 253, 131, 202, 207, 116, 89, 18]);

export interface BorrowInstructionAccounts {
    signer: Address;
    dex: Address;
    user: Address;
    position: Address;
...
export interface BorrowInstructionArgs {
    token0Amt: number | bigint;
    token1Amt: number | bigint;
    maxShares: number | bigint;
}

function getBorrowInstructionDataEncoder(): Encoder<BorrowInstructionArgs> {
...
function getBorrowInstructionDataDecoder(): Decoder<BorrowInstructionArgs> {
...
export interface ParsedBorrowInstruction {
    programId: Address;
    accounts: {
        signer: AccountMeta;
        dex: AccountMeta;
    ...
    data: BorrowInstructionArgs;
}

export function parseBorrowInstruction(instruction: TransactionInstruction): ParsedBorrowInstruction {
    if (instruction.keys.length < 26) {
        throw new Error('Expected 26 account metas for Borrow instruction');
    }
    if (!BORROW_INSTRUCTION_DISCRIMINATOR.every((byte, index) => instruction.data[0 + index] === byte)) {
        throw new Error('Borrow instruction discriminator mismatch');
    }
    ...
    return {
        ...
        data: getBorrowInstructionDataDecoder().decode(instructionData),
    };
}
...
export function createBorrowInstruction(
    ...
    let data = Buffer.from(getBorrowInstructionDataEncoder().encode(args));
    data = Buffer.concat([
        data.subarray(0, 0),
        Buffer.alloc(Math.max(0, 0 - data.length)),
        Buffer.from(BORROW_INSTRUCTION_DISCRIMINATOR),
        data.subarray(0),
    ]);
