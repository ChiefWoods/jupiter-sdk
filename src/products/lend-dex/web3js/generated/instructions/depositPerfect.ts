...
export const DEPOSIT_PERFECT_INSTRUCTION_DISCRIMINATOR = new Uint8Array([4, 98, 66, 216, 110, 126, 154, 8]);
...
function getDepositPerfectInstructionDataEncoder(): Encoder<DepositPerfectInstructionArgs> {
...
function getDepositPerfectInstructionDataDecoder(): Decoder<DepositPerfectInstructionArgs> {
...
export interface ParsedDepositPerfectInstruction {
    programId: Address;
    accounts: {
        signer: AccountMeta;
        dex: AccountMeta;
    ...
    data: DepositPerfectInstructionArgs;
}

export function parseDepositPerfectInstruction(instruction: TransactionInstruction): ParsedDepositPerfectInstruction {
    if (instruction.keys.length < 26) {
        throw new Error('Expected 26 account metas for DepositPerfect instruction');
    }
    if (!DEPOSIT_PERFECT_INSTRUCTION_DISCRIMINATOR.every((byte, index) => instruction.data[0 + index] === byte)) {
        throw new Error('DepositPerfect instruction discriminator mismatch');
    }
    ...
    return {
        ...
        data: getDepositPerfectInstructionDataDecoder().decode(instructionData),
    };
}
...
export function createDepositPerfectInstruction(
    ...
    let data = Buffer.from(getDepositPerfectInstructionDataEncoder().encode(args));
    data = Buffer.concat([
        data.subarray(0, 0),
        Buffer.alloc(Math.max(0, 0 - data.length)),
        Buffer.from(DEPOSIT_PERFECT_INSTRUCTION_DISCRIMINATOR),
        data.subarray(0),
    ]);
