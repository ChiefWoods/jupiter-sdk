/**
 * This code was AUTOGENERATED using the Codama library.
 * Please DO NOT EDIT THIS FILE, instead use visitors
 * to add features, then rerun Codama to update it.
 *
 * @see https://github.com/codama-idl/codama
 */

import {
  combineCodec,
  fixDecoderSize,
  fixEncoderSize,
  getArrayDecoder,
  getArrayEncoder,
  getBytesDecoder,
  getBytesEncoder,
  getStructDecoder,
  getStructEncoder,
  getU16Decoder,
  getU16Encoder,
  getU64Decoder,
  getU64Encoder,
  getU8Decoder,
  getU8Encoder,
  SOLANA_ERROR__PROGRAM_CLIENTS__INSUFFICIENT_ACCOUNT_METAS,
  SolanaError,
  transformEncoder,
  type AccountMeta,
  type AccountSignerMeta,
  type Address,
  type Codec,
  type Decoder,
  type Encoder,
  type Instruction,
  type InstructionWithAccounts,
  type InstructionWithData,
  type ReadonlyAccount,
  type ReadonlySignerAccount,
  type ReadonlyUint8Array,
  type TransactionSigner,
  type WritableAccount,
} from "@solana/kit";
import {
  getAccountMetaFactory,
  type ResolvedInstructionAccount,
} from "@solana/kit/program-client-core";
import { AGGREGATOR_V6_PROGRAM_ADDRESS } from "../programs";
import {
  getRoutePlanStepDecoder,
  getRoutePlanStepEncoder,
  type RoutePlanStep,
  type RoutePlanStepArgs,
} from "../types";

export const SHARED_ACCOUNTS_EXACT_OUT_ROUTE_DISCRIMINATOR: ReadonlyUint8Array =
  new Uint8Array([176, 209, 105, 168, 154, 125, 69, 62]);

export function getSharedAccountsExactOutRouteDiscriminatorBytes(): ReadonlyUint8Array {
  return fixEncoderSize(getBytesEncoder(), 8).encode(
    SHARED_ACCOUNTS_EXACT_OUT_ROUTE_DISCRIMINATOR,
  );
}

export type SharedAccountsExactOutRouteInstruction<
  TProgram extends string = typeof AGGREGATOR_V6_PROGRAM_ADDRESS,
  TAccountTokenProgram extends string | AccountMeta<string> =
    "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA",
  TAccountProgramAuthority extends string | AccountMeta<string> = string,
  TAccountUserTransferAuthority extends string | AccountMeta<string> = string,
  TAccountSourceTokenAccount extends string | AccountMeta<string> = string,
  TAccountProgramSourceTokenAccount extends string | AccountMeta<string> =
    string,
  TAccountProgramDestinationTokenAccount extends string | AccountMeta<string> =
    string,
  TAccountDestinationTokenAccount extends string | AccountMeta<string> = string,
  TAccountSourceMint extends string | AccountMeta<string> = string,
  TAccountDestinationMint extends string | AccountMeta<string> = string,
  TAccountPlatformFeeAccount extends string | AccountMeta<string> = string,
  TAccountToken2022Program extends string | AccountMeta<string> = string,
  TAccountEventAuthority extends string | AccountMeta<string> =
    "D8cy77BBepLMngZx6ZukaTff5hCt1HrWyKk3Hnd9oitf",
  TAccountProgram extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountTokenProgram extends string
        ? ReadonlyAccount<TAccountTokenProgram>
        : TAccountTokenProgram,
      TAccountProgramAuthority extends string
        ? ReadonlyAccount<TAccountProgramAuthority>
        : TAccountProgramAuthority,
      TAccountUserTransferAuthority extends string
        ? ReadonlySignerAccount<TAccountUserTransferAuthority> &
            AccountSignerMeta<TAccountUserTransferAuthority>
        : TAccountUserTransferAuthority,
      TAccountSourceTokenAccount extends string
        ? WritableAccount<TAccountSourceTokenAccount>
        : TAccountSourceTokenAccount,
      TAccountProgramSourceTokenAccount extends string
        ? WritableAccount<TAccountProgramSourceTokenAccount>
        : TAccountProgramSourceTokenAccount,
      TAccountProgramDestinationTokenAccount extends string
        ? WritableAccount<TAccountProgramDestinationTokenAccount>
        : TAccountProgramDestinationTokenAccount,
      TAccountDestinationTokenAccount extends string
        ? WritableAccount<TAccountDestinationTokenAccount>
        : TAccountDestinationTokenAccount,
      TAccountSourceMint extends string
        ? ReadonlyAccount<TAccountSourceMint>
        : TAccountSourceMint,
      TAccountDestinationMint extends string
        ? ReadonlyAccount<TAccountDestinationMint>
        : TAccountDestinationMint,
      TAccountPlatformFeeAccount extends string
        ? WritableAccount<TAccountPlatformFeeAccount>
        : TAccountPlatformFeeAccount,
      TAccountToken2022Program extends string
        ? ReadonlyAccount<TAccountToken2022Program>
        : TAccountToken2022Program,
      TAccountEventAuthority extends string
        ? ReadonlyAccount<TAccountEventAuthority>
        : TAccountEventAuthority,
      TAccountProgram extends string
        ? ReadonlyAccount<TAccountProgram>
        : TAccountProgram,
      ...TRemainingAccounts,
    ]
  >;

export type SharedAccountsExactOutRouteInstructionData = {
  discriminator: ReadonlyUint8Array;
  id: number;
  routePlan: Array<RoutePlanStep>;
  outAmount: bigint;
  quotedInAmount: bigint;
  slippageBps: number;
  platformFeeBps: number;
};

export type SharedAccountsExactOutRouteInstructionDataArgs = {
  id: number;
  routePlan: Array<RoutePlanStepArgs>;
  outAmount: number | bigint;
  quotedInAmount: number | bigint;
  slippageBps: number;
  platformFeeBps: number;
};

export function getSharedAccountsExactOutRouteInstructionDataEncoder(): Encoder<SharedAccountsExactOutRouteInstructionDataArgs> {
  return transformEncoder(
    getStructEncoder([
      ["discriminator", fixEncoderSize(getBytesEncoder(), 8)],
      ["id", getU8Encoder()],
      ["routePlan", getArrayEncoder(getRoutePlanStepEncoder())],
      ["outAmount", getU64Encoder()],
      ["quotedInAmount", getU64Encoder()],
      ["slippageBps", getU16Encoder()],
      ["platformFeeBps", getU8Encoder()],
    ]),
    (value) => ({
      ...value,
      discriminator: SHARED_ACCOUNTS_EXACT_OUT_ROUTE_DISCRIMINATOR,
    }),
  );
}

export function getSharedAccountsExactOutRouteInstructionDataDecoder(): Decoder<SharedAccountsExactOutRouteInstructionData> {
  return getStructDecoder([
    ["discriminator", fixDecoderSize(getBytesDecoder(), 8)],
    ["id", getU8Decoder()],
    ["routePlan", getArrayDecoder(getRoutePlanStepDecoder())],
    ["outAmount", getU64Decoder()],
    ["quotedInAmount", getU64Decoder()],
    ["slippageBps", getU16Decoder()],
    ["platformFeeBps", getU8Decoder()],
  ]);
}

export function getSharedAccountsExactOutRouteInstructionDataCodec(): Codec<
  SharedAccountsExactOutRouteInstructionDataArgs,
  SharedAccountsExactOutRouteInstructionData
> {
  return combineCodec(
    getSharedAccountsExactOutRouteInstructionDataEncoder(),
    getSharedAccountsExactOutRouteInstructionDataDecoder(),
  );
}

export type SharedAccountsExactOutRouteInput<
  TAccountTokenProgram extends string = string,
  TAccountProgramAuthority extends string = string,
  TAccountUserTransferAuthority extends string = string,
  TAccountSourceTokenAccount extends string = string,
  TAccountProgramSourceTokenAccount extends string = string,
  TAccountProgramDestinationTokenAccount extends string = string,
  TAccountDestinationTokenAccount extends string = string,
  TAccountSourceMint extends string = string,
  TAccountDestinationMint extends string = string,
  TAccountPlatformFeeAccount extends string = string,
  TAccountToken2022Program extends string = string,
  TAccountEventAuthority extends string = string,
  TAccountProgram extends string = string,
> = {
  tokenProgram?: Address<TAccountTokenProgram>;
  programAuthority: Address<TAccountProgramAuthority>;
  userTransferAuthority: TransactionSigner<TAccountUserTransferAuthority>;
  sourceTokenAccount: Address<TAccountSourceTokenAccount>;
  programSourceTokenAccount: Address<TAccountProgramSourceTokenAccount>;
  programDestinationTokenAccount: Address<TAccountProgramDestinationTokenAccount>;
  destinationTokenAccount: Address<TAccountDestinationTokenAccount>;
  sourceMint: Address<TAccountSourceMint>;
  destinationMint: Address<TAccountDestinationMint>;
  platformFeeAccount?: Address<TAccountPlatformFeeAccount>;
  token2022Program?: Address<TAccountToken2022Program>;
  eventAuthority?: Address<TAccountEventAuthority>;
  program: Address<TAccountProgram>;
  id: SharedAccountsExactOutRouteInstructionDataArgs["id"];
  routePlan: SharedAccountsExactOutRouteInstructionDataArgs["routePlan"];
  outAmount: SharedAccountsExactOutRouteInstructionDataArgs["outAmount"];
  quotedInAmount: SharedAccountsExactOutRouteInstructionDataArgs["quotedInAmount"];
  slippageBps: SharedAccountsExactOutRouteInstructionDataArgs["slippageBps"];
  platformFeeBps: SharedAccountsExactOutRouteInstructionDataArgs["platformFeeBps"];
};

export function getSharedAccountsExactOutRouteInstruction<
  TAccountTokenProgram extends string,
  TAccountProgramAuthority extends string,
  TAccountUserTransferAuthority extends string,
  TAccountSourceTokenAccount extends string,
  TAccountProgramSourceTokenAccount extends string,
  TAccountProgramDestinationTokenAccount extends string,
  TAccountDestinationTokenAccount extends string,
  TAccountSourceMint extends string,
  TAccountDestinationMint extends string,
  TAccountPlatformFeeAccount extends string,
  TAccountToken2022Program extends string,
  TAccountEventAuthority extends string,
  TAccountProgram extends string,
  TProgramAddress extends Address = typeof AGGREGATOR_V6_PROGRAM_ADDRESS,
>(
  input: SharedAccountsExactOutRouteInput<
    TAccountTokenProgram,
    TAccountProgramAuthority,
    TAccountUserTransferAuthority,
    TAccountSourceTokenAccount,
    TAccountProgramSourceTokenAccount,
    TAccountProgramDestinationTokenAccount,
    TAccountDestinationTokenAccount,
    TAccountSourceMint,
    TAccountDestinationMint,
    TAccountPlatformFeeAccount,
    TAccountToken2022Program,
    TAccountEventAuthority,
    TAccountProgram
  >,
  config?: { programAddress?: TProgramAddress },
): SharedAccountsExactOutRouteInstruction<
  TProgramAddress,
  TAccountTokenProgram,
  TAccountProgramAuthority,
  TAccountUserTransferAuthority,
  TAccountSourceTokenAccount,
  TAccountProgramSourceTokenAccount,
  TAccountProgramDestinationTokenAccount,
  TAccountDestinationTokenAccount,
  TAccountSourceMint,
  TAccountDestinationMint,
  TAccountPlatformFeeAccount,
  TAccountToken2022Program,
  TAccountEventAuthority,
  TAccountProgram
> {
  // Program address.
  const programAddress =
    config?.programAddress ?? AGGREGATOR_V6_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    tokenProgram: { value: input.tokenProgram ?? null, isWritable: false },
    programAuthority: {
      value: input.programAuthority ?? null,
      isWritable: false,
    },
    userTransferAuthority: {
      value: input.userTransferAuthority ?? null,
      isWritable: false,
    },
    sourceTokenAccount: {
      value: input.sourceTokenAccount ?? null,
      isWritable: true,
    },
    programSourceTokenAccount: {
      value: input.programSourceTokenAccount ?? null,
      isWritable: true,
    },
    programDestinationTokenAccount: {
      value: input.programDestinationTokenAccount ?? null,
      isWritable: true,
    },
    destinationTokenAccount: {
      value: input.destinationTokenAccount ?? null,
      isWritable: true,
    },
    sourceMint: { value: input.sourceMint ?? null, isWritable: false },
    destinationMint: {
      value: input.destinationMint ?? null,
      isWritable: false,
    },
    platformFeeAccount: {
      value: input.platformFeeAccount ?? null,
      isWritable: true,
    },
    token2022Program: {
      value: input.token2022Program ?? null,
      isWritable: false,
    },
    eventAuthority: { value: input.eventAuthority ?? null, isWritable: false },
    program: { value: input.program ?? null, isWritable: false },
  };
  const accounts = originalAccounts as Record<
    keyof typeof originalAccounts,
    ResolvedInstructionAccount
  >;

  // Original args.
  const args = { ...input };

  // Resolve default values.
  if (!accounts.tokenProgram.value) {
    accounts.tokenProgram.value =
      "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA" as Address<"TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA">;
  }
  if (!accounts.eventAuthority.value) {
    accounts.eventAuthority.value =
      "D8cy77BBepLMngZx6ZukaTff5hCt1HrWyKk3Hnd9oitf" as Address<"D8cy77BBepLMngZx6ZukaTff5hCt1HrWyKk3Hnd9oitf">;
  }

  const getAccountMeta = getAccountMetaFactory(programAddress, "programId");
  return Object.freeze({
    accounts: [
      getAccountMeta("tokenProgram", accounts.tokenProgram),
      getAccountMeta("programAuthority", accounts.programAuthority),
      getAccountMeta("userTransferAuthority", accounts.userTransferAuthority),
      getAccountMeta("sourceTokenAccount", accounts.sourceTokenAccount),
      getAccountMeta(
        "programSourceTokenAccount",
        accounts.programSourceTokenAccount,
      ),
      getAccountMeta(
        "programDestinationTokenAccount",
        accounts.programDestinationTokenAccount,
      ),
      getAccountMeta(
        "destinationTokenAccount",
        accounts.destinationTokenAccount,
      ),
      getAccountMeta("sourceMint", accounts.sourceMint),
      getAccountMeta("destinationMint", accounts.destinationMint),
      getAccountMeta("platformFeeAccount", accounts.platformFeeAccount),
      getAccountMeta("token2022Program", accounts.token2022Program),
      getAccountMeta("eventAuthority", accounts.eventAuthority),
      getAccountMeta("program", accounts.program),
    ],
    data: getSharedAccountsExactOutRouteInstructionDataEncoder().encode(
      args as SharedAccountsExactOutRouteInstructionDataArgs,
    ),
    programAddress,
  } as SharedAccountsExactOutRouteInstruction<
    TProgramAddress,
    TAccountTokenProgram,
    TAccountProgramAuthority,
    TAccountUserTransferAuthority,
    TAccountSourceTokenAccount,
    TAccountProgramSourceTokenAccount,
    TAccountProgramDestinationTokenAccount,
    TAccountDestinationTokenAccount,
    TAccountSourceMint,
    TAccountDestinationMint,
    TAccountPlatformFeeAccount,
    TAccountToken2022Program,
    TAccountEventAuthority,
    TAccountProgram
  >);
}

export type ParsedSharedAccountsExactOutRouteInstruction<
  TProgram extends string = typeof AGGREGATOR_V6_PROGRAM_ADDRESS,
  TAccountMetas extends readonly AccountMeta[] = readonly AccountMeta[],
> = {
  programAddress: Address<TProgram>;
  accounts: {
    tokenProgram: TAccountMetas[0];
    programAuthority: TAccountMetas[1];
    userTransferAuthority: TAccountMetas[2];
    sourceTokenAccount: TAccountMetas[3];
    programSourceTokenAccount: TAccountMetas[4];
    programDestinationTokenAccount: TAccountMetas[5];
    destinationTokenAccount: TAccountMetas[6];
    sourceMint: TAccountMetas[7];
    destinationMint: TAccountMetas[8];
    platformFeeAccount?: TAccountMetas[9] | undefined;
    token2022Program?: TAccountMetas[10] | undefined;
    eventAuthority: TAccountMetas[11];
    program: TAccountMetas[12];
  };
  data: SharedAccountsExactOutRouteInstructionData;
};

export function parseSharedAccountsExactOutRouteInstruction<
  TProgram extends string,
  TAccountMetas extends readonly AccountMeta[],
>(
  instruction: Instruction<TProgram> &
    InstructionWithAccounts<TAccountMetas> &
    InstructionWithData<ReadonlyUint8Array>,
): ParsedSharedAccountsExactOutRouteInstruction<TProgram, TAccountMetas> {
  if (instruction.accounts.length < 13) {
    throw new SolanaError(
      SOLANA_ERROR__PROGRAM_CLIENTS__INSUFFICIENT_ACCOUNT_METAS,
      {
        actualAccountMetas: instruction.accounts.length,
        expectedAccountMetas: 13,
      },
    );
  }
  let accountIndex = 0;
  const getNextAccount = () => {
    const accountMeta = (instruction.accounts as TAccountMetas)[accountIndex]!;
    accountIndex += 1;
    return accountMeta;
  };
  const getNextOptionalAccount = () => {
    const accountMeta = getNextAccount();
    return accountMeta.address === AGGREGATOR_V6_PROGRAM_ADDRESS
      ? undefined
      : accountMeta;
  };
  return {
    programAddress: instruction.programAddress,
    accounts: {
      tokenProgram: getNextAccount(),
      programAuthority: getNextAccount(),
      userTransferAuthority: getNextAccount(),
      sourceTokenAccount: getNextAccount(),
      programSourceTokenAccount: getNextAccount(),
      programDestinationTokenAccount: getNextAccount(),
      destinationTokenAccount: getNextAccount(),
      sourceMint: getNextAccount(),
      destinationMint: getNextAccount(),
      platformFeeAccount: getNextOptionalAccount(),
      token2022Program: getNextOptionalAccount(),
      eventAuthority: getNextAccount(),
      program: getNextAccount(),
    },
    data: getSharedAccountsExactOutRouteInstructionDataDecoder().decode(
      instruction.data,
    ),
  };
}
