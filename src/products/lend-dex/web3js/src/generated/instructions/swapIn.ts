...
const SwapInInstructionDataCodec = getStructCodec([
    ['swap0to1', getBooleanCodec()],
    ['amountIn', getU64Codec()],
    ['amountOutMin', getU64Codec()],
]);
...
export function createSwapInInstruction(
    ...
    const instructionData = Buffer.from(SwapInInstructionDataCodec.encode(args));
    const discriminator = Buffer.from('8dac0ad04509389a', 'hex');
